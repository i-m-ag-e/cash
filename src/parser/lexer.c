...
static const bool PUNCTUATION[128] = {
    ...
    ['$'] = true,  ['\t'] = true, ['\n'] = true, [' '] = true};
// ">|<()'\";&`$\t\n"
static const bool VALID_SUB_START[128] = {
...
struct Token lexer_next_token(struct Lexer* lexer) {
    if (lexer->repl_mode) {
        ...
        struct Token token = lexer_pop_token(lexer);
        return token;
    }
...
void lexer_lex_full(struct Lexer* lexer) {
    while (true) {
        struct Token token = lexer_lex(lexer);
...
static struct Token consume_string(struct Lexer* lexer) {
    ...
    while (true) {
        ...
        else if (!is_at_end(lexer) && !PUNCTUATION[(int)c])
...
static void consume_unquoted_string(struct Lexer* lexer) {
    ...
    int escapes = 0;
    while (!is_at_end(lexer) && !PUNCTUATION[(int)(c = peek(lexer))]) {
        if (c == '\\') {
            escapes++;
            advance(lexer);
        }
...
static struct Token lexer_pop_token(struct Lexer* lexer) {
    struct Token token = lexer->token_queue[lexer->token_queue_head];
